...
def costdata():
    if request.method == 'POST':
        ...
        if request.form.get('submit_button') == 'submit':
            ...
            if Cloud == "AWS":
                node_cost = spreadsheet.aws_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
            elif Cloud == "GCP":
                node_cost = spreadsheet.gcp_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
            else:
                node_cost = spreadsheet.azure_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
            ...
            aws_node_cost = spreadsheet.aws_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
            gcp_node_cost = spreadsheet.gcp_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
            azure_node_cost = spreadsheet.azure_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes)
