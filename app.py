...
def costdata():
    if request.method == 'POST':
        ...
        if request.form.get('submit_button') == 'submit':
            ...
            if Cloud == "AWS":
                node_cost,store_cost = spreadsheet.aws_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes,StorageType,Number_of_Storage_Units)
            elif Cloud == "GCP":
                node_cost,store_cost = spreadsheet.gcp_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes,StorageType,Number_of_Storage_Units)
            else:
            ...
            return render_template('index.html',title="Dashboard",tables=[node_cost.head().to_html(index = False),store_cost.head().to_html(index=False)],titles = [ "","{} Node Data".format(Cloud),"{} Storage Data".format(Cloud)])

        elif request.form.get('submit_button')=="compare":
            aws_node_cost,aws_store_cost = spreadsheet.aws_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes,StorageType,Number_of_Storage_Units)
            gcp_node_cost,gcp_store_cost = spreadsheet.gcp_cost(RAM,vCPUs,OS,StorageUnitSize,Cloud,Number_of_Nodes,StorageType,Number_of_Storage_Units)
